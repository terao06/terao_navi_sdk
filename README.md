...
A: 以下を確認してください：
- `chat.js` のURLが正しいか
- `data-credential` が正しく設定されているか
- コンソールにエラーメッセージが出ていないか
- ブラウザのiframe制限やCORSエラーがないか

**Q: 認証エラーが発生する**

A: 以下を確認してください：
- credentialが正しくBase64エンコードされているか
- clientIdとclientSecretが有効か
- Terao Navi APIのドメイン制限設定を確認
...
- ネットワーク接続を確認
- ブラウザのコンソールでエラーを確認
- `/api/chat` エンドポイントが正常に動作しているか

**Q: レスポンスが遅い**

A: 以下を確認してください：
- Terao Navi APIのレスポンス時間を確認
- ネットワーク遅延を確認
- サーバーのリソース使用状況を確認
